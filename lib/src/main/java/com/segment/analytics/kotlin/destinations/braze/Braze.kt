...
// A Destination plugin that adds session tracking to Braze cloud mode.
class BrazeSession : Plugin, VersionedPlugin {

    override val type: Plugin.Type = Plugin.Type.Enrichment
    override lateinit var analytics: Analytics
    var sessionID: Long = -1
    private val key = "Actions Braze"
    ...
    // Add the session_id to the Braze payload for cloud mode to handle.
    private inline fun <reified T : BaseEvent?> insertSession(payload: T?): BaseEvent? {
        var returnPayload = payload
        payload?.let {
            analytics.log(
                message = "Running ${payload.type} payload through BrazeSession",
    ...
    override fun execute(event: BaseEvent): BaseEvent? {
        if (!active) { // If braze destination is disabled, no need to do this enrichment
            return event
        }
